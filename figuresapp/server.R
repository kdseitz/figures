...
shinyServer(function(input, output, session) {
  gen.all <- function(x, y, z, g=z) {
    ...
    createaverage <- function(x) {
      if (((length(grep(
        ...
        response <-
          ...
          ))) / (((length(na.exclude(x)))-(length(grep('not applicable',x,ignore.case=TRUE))
          )) - 1))
      } else{
        if (((length(
          grep('difficult', x, ignore.case = TRUE)
    ...
    folder<-as.character(g)
    dir.create(folder)
    setwd(paste('C:', 'Users', Sys.getenv('USERNAME'), 'Documents', folder, sep = '/'))
  ...
  observeEvent(input$run, {
    gen.all(input$file, input$pop, input$path, input$save)
    output$done <-
